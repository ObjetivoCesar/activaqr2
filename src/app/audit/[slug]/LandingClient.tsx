...
export default function LandingClient({ 
  ...
  return (
    ...
    >
      ...
      {/* --- NAVBAR DINÁMICO --- */}
      <nav 
        className={`fixed top-0 left-0 right-0 z-50 h-20 px-6 md:px-12 flex items-center justify-between transition-all duration-500 backdrop-blur-md ${
          scrolled ? 'shadow-2xl' : 'bg-transparent border-b border-white/5'
        }`}
        style={{ 
          backgroundColor: scrolled ? brand : 'transparent',
          color: scrolled ? brandContrast : 'white',
          borderBottom: scrolled ? `2px solid ${brandContrast}20` : '1px solid rgba(255,255,255,0.05)'
        }}
      >
        <div className="flex items-center gap-4">
          {tenant.logo_url ? (
            <img src={tenant.logo_url} alt={tenant.name} className={`h-10 md:h-12 w-auto object-contain ${scrolled && brandContrast === '#000000' ? '' : 'brightness-0 invert'}`} />
          ) : (
            <span className="text-xl font-black uppercase tracking-tighter">
              {tenant.name} <span style={{ color: scrolled ? brandContrast : brand }}>.</span>
            </span>
          )}
        </div>
        <div className="hidden md:flex items-center gap-8">
          <Link href="#servicios" className={`text-[10px] font-black uppercase tracking-[0.2em] transition-colors ${scrolled ? 'opacity-80 hover:opacity-100' : 'text-white/70 hover:text-white'}`}>Servicios</Link>
          <Link href="#unidades" className={`text-[10px] font-black uppercase tracking-[0.2em] transition-colors ${scrolled ? 'opacity-80 hover:opacity-100' : 'text-white/70 hover:text-white'}`}>Flota</Link>
          <Link href="#contacto" className={`text-[10px] font-black uppercase tracking-[0.2em] transition-colors ${scrolled ? 'opacity-80 hover:opacity-100' : 'text-white/70 hover:text-white'}`}>Contacto</Link>
        </div>
        <Link href={`https://wa.me/${tenant.whatsapp_number}`} target="_blank" className={`px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all hover:scale-105 border-2 ${scrolled ? 'border-current' : ''}`} style={{ backgroundColor: scrolled ? 'transparent' : brand, color: scrolled ? 'inherit' : brandContrast, borderColor: scrolled ? 'inherit' : 'transparent' }}>Soporte 24/7</Link>
      </nav>
          ...
          <div className="max-w-4xl space-y-8">
            ...
            <h1 className="text-5xl md:text-8xl font-black text-white leading-[0.95] tracking-tighter uppercase">
              {copy.heroTitle.split(' ').map((word: string, i: number) => {
                const isHighlight = word.toLowerCase() === 'logistics' || word.toLowerCase() === 'luxury' || word.toLowerCase() === 'premium' || word.toLowerCase() === 'elite';
                if (isHighlight) {
                  return (
            ...
            <div className="flex flex-wrap gap-4 pt-6">
              ...
              <Link href="#servicios" className="px-10 py-5 rounded-2xl text-xs font-black uppercase tracking-widest flex items-center gap-3 transition-all hover:bg-white/10 border-2" style={{ borderColor: brand, color: brand }}>Explorar <ChevronRight size={18} /></Link>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            ...
            <div className="relative group">
              ...
              <div className="relative rounded-[40px] overflow-hidden aspect-[4/5] shadow-2xl">
                <img src={aboutImgs[activeAboutImg]} className="w-full h-full object-cover transition-opacity duration-700" />
          ...
          <div className={`flex gap-6 pb-8 snap-x snap-mandatory hide-scrollbar ${layoutType === 'luxury' ? 'justify-center overflow-visible' : 'overflow-x-auto md:grid md:grid-cols-2 lg:grid-cols-4'}`}>
            {services.map((svc: string, i: number) => (
              <div key={i} className={`snap-center shrink-0 relative rounded-[32px] overflow-hidden group shadow-2xl transition-all duration-500 ${layoutType === 'luxury' ? 'min-w-[300px] h-[500px] hover:-translate-y-4' : 'min-w-[85vw] md:min-w-0 h-[450px]'}`}>
                <div className="absolute inset-0 bg-cover bg-center transition-transform duration-700 group-hover:scale-110" style={{ backgroundImage: `url('https://images.unsplash.com/photo-1519003722824-194d4455a60c?q=80&w=600&auto=format&fit=crop')` }} />
      ...
      {/* --- VIDEO SECTION --- */}
      {vcard.video_url && (
        <section id="video" className={`bg-slate-900 text-white overflow-hidden ${layoutType === 'luxury' ? 'py-0 h-[80vh]' : 'py-24'}`}>
      ...
      <section className="py-24 md:py-32 bg-slate-950 text-white">
        <div className="container mx-auto px-6 text-center space-y-12">
           <div className="space-y-4">
              <div className="flex justify-center gap-1">{[...Array(5)].map((_, i) => <Star key={i} size={24} fill="#fbbf24" className="text-[#fbbf24]" />)}</div>
              <p className="text-8xl md:text-[140px] font-black tracking-tighter leading-none text-white">{vcard.rating || '5.0'}</p>
      ...
      <section id="contacto" className={`py-24 md:py-32 bg-white ${layoutType === 'fleet' ? 'pb-0' : ''}`}>
        ...
        {layoutType === 'fleet' && (
          <div className="w-full h-[500px] mt-24 relative">
             ...
             <div className="absolute inset-0 flex items-center justify-center">
                <Link href={vcard.google_maps_url || '#'} target="_blank" className="px-12 py-6 bg-white text-slate-900 rounded-2xl text-xs font-black uppercase tracking-widest shadow-2xl hover:scale-110 transition-all">Explorar Cobertura</Link>
             </div>
          </div>
        )}
      </section>
      ...
      {/* --- FOOTER DINÁMICO --- */}
      <footer 
        className="py-16 text-center border-t transition-all duration-500" 
        style={{ 
          backgroundColor: brand, 
          color: brandContrast,
          borderColor: `${brandContrast}10`
        }}
      >
        <div className="container mx-auto px-6 space-y-6">
           <p className="text-[10px] font-black uppercase tracking-[0.5em] opacity-60">© {new Date().getFullYear()} {tenant.name} | ActivaQR Pro</p>
           <div className="flex justify-center gap-4 opacity-40">
              <Shield size={16} /> <Zap size={16} /> <Award size={16} />
           </div>
        </div>
      </footer>
    </div>
